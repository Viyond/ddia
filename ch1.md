...
- 存储数据，以便自己或其他应用程序之后能再次找到 ***（database,数据库）***
- 记住开销昂贵操作的结果，加快读取速度 ***（cache,缓存）***
- 允许用户按关键字搜索数据，或以各种方式对数据进行过滤 ***（search indexes,搜索索引）***
- 向其他进程发送消息，进行异步处理 ***（stream processing,流处理）***
- 定期处理累积的大批量数据 ***（batch processing,批处理）***
